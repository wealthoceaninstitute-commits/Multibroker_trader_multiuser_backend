...
export default function Positions() {
  ...
  const fetchAll = async () => {
    ...
    try {
      const res = await api.get(`/get_positions?userid=${userid}`, {
  ...
  const closeSelected = async () => {
    ...
    try {
      ...
      const res = await api.post('/close_position', {
            ...
            return (
              <tr key={rowId} data-rowid={rowId}>
                ...
                <td
                  style={{
                    color:
                      (parseFloat(row.net_profit) || 0) < 0 ? 'red' : 'green',
                    fontWeight: 'bold',
                  }}
                >
                  {row.net_profit ?? 'N/A'}
  ...
  return (
    <Card className="p-3">
      <div className="mb-3 d-flex gap-2 align-items-center">
        ...
        <Button variant="danger" onClick={closeSelected}>
          Close Position
        </Button>
      ...
      <Tabs defaultActiveKey="open" className="mb-3">
        <Tab eventKey="open" title="Open">
          {renderTable(openRows, 'open_positions_table')}
        </Tab>
        <Tab eventKey="closed" title="Closed">
          {renderTable(closedRows, 'closed_positions_table')}
        </Tab>
      </Tabs>
    </Card>
  );
